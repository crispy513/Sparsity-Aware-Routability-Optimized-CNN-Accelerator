...
always_ff @(posedge clk) begin
    if (rst) asic_en_q <= 1'b0;
    else asic_en_q <= asic_en;
end
...
always_ff @(posedge clk) begin
    ...
    else if (sparse_dma_done_fire) begin
        case (ctrl_DMA_mode)
            `MODE_IFMAP: begin
                ...
                ifmap_glb_word_offset <= ifmap_glb_word_offset + sparse_dma_len[`GLB_ADDR_BITS-3:0];
            end
            `MODE_FILTER: begin
                ...
                filter_glb_word_offset <= filter_glb_word_offset + sparse_dma_len[`GLB_ADDR_BITS-3:0];
            end
            `MODE_OFMAP: begin
...
BitmapBuffer #(
    ...
    .start_stream(1'b0),
    .stream_base(`SPARSE_BMAP_ADDR_BITS'd0),
    .stream_len({(`SPARSE_BMAP_ADDR_BITS+1){1'b0}}),
    .stream_busy(),
    .stream_done(),
    .dec_bitmap_o(),
    .dec_nz_count_o(),
...
assign opsum_store_sparse = (GLB_DI_select == `GLB_DO_PSUM) && (ctrl_GLB_WEB == 1'b0);
...
always_ff @(posedge clk) begin
    if (rst || sparse_start) begin
        ...
        if (sparse_selected_consumes_bitmap) begin
...
always_comb begin
    ...
    if (sparse_dma_active) begin
        ...
        case (ctrl_DMA_mode)
            `MODE_IFMAP: begin
                ...
                DMA_GLB_ADDR = {ifmap_glb_word_offset, 2'd0};
                DMA_byte_bias = 2'd0;
            end
            `MODE_FILTER: begin
                DMA_mode = `MODE_BIAS;
                DMA_DRAM_ADDR = filter_addr_i + {filter_dma_word_offset[`AXI_ADDR_BITS-3:0], 2'd0};
                DMA_GLB_ADDR = GLB_filter_addr_i + {filter_glb_word_offset, 2'd0};
                DMA_byte_bias = 2'd0;
            end
            `MODE_OFMAP: begin
                DMA_mode = `MODE_OFMAP;
    ...
    GLB_data_in_sparse = (sparse_selected_pad) ? 32'h80808080 :
                         (sparse_read_sel == `SPARSE_READ_NONE) ? GLB_DO :
