...
struct runtime_info {
    ...
    uint64_t reset_cycles;        // 系統重置 (Reset) 開銷
    uint64_t mmio_setup_cycles;   // 暫存器設定 (MMIO) 開銷
    uint64_t dma_read_cycles;     // DMA 讀取等待時間
    uint64_t dma_write_cycles;    // DMA 寫入等待時間
    uint64_t pe_active_cycles;    // PE 陣列真實運算的週期
};
