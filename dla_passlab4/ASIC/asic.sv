...
/*logic [$bits(FT_CONTROLLER.asic_controller_0.valid_e)-1:0]               dbg_prev_valid_e;
logic [$bits(PE_en)-1:0]                 dbg_prev_PE_en;
logic [$bits(opsum_tag_X)-1:0]           dbg_prev_opsum_tag_X;
logic [$bits(opsum_tag_Y)-1:0]           dbg_prev_opsum_tag_Y;
logic [$bits(PEA_opsum_issue_fire)-1:0]  dbg_prev_PEA_opsum_issue_fire;
logic [$bits(FT_CONTROLLER.asic_controller_0.opsum_meta_count)-1:0]      dbg_prev_opsum_meta_count;
logic [$bits(PEA_opsum_valid)-1:0]       dbg_prev_PEA_opsum_valid;
logic [$bits(PEA_opsum_ready)-1:0]       dbg_prev_PEA_opsum_ready;

logic [$bits(PE_array.GON_opsum_valid)-1:0] dbg_prev_GON_opsum_valid;
logic [$bits(PE_array.GON_opsum_ready)-1:0] dbg_prev_GON_opsum_ready;
logic [7:0] dbg_prev_PE_row2_valid;
logic [7:0] dbg_prev_PE_row5_valid;

always_ff @(posedge clk) begin
    if (rst) begin
        dbg_prev_valid_e              <= '0;
        dbg_prev_PE_en                <= '0;
        dbg_prev_opsum_tag_X          <= '0;
        dbg_prev_opsum_tag_Y          <= '0;
        dbg_prev_PEA_opsum_issue_fire <= '0;
        dbg_prev_opsum_meta_count     <= '0;
        dbg_prev_PEA_opsum_valid      <= '0;
        dbg_prev_PEA_opsum_ready      <= '0;

        dbg_prev_GON_opsum_valid      <= '0;
        dbg_prev_GON_opsum_ready      <= '0;
        dbg_prev_PE_row2_valid        <= '0;
        dbg_prev_PE_row5_valid        <= '0;
    end else begin
        if ((FT_CONTROLLER.asic_controller_0.valid_e                      !== dbg_prev_valid_e)              ||
            (PE_en                        !== dbg_prev_PE_en)                ||
            (opsum_tag_X                  !== dbg_prev_opsum_tag_X)          ||
            (opsum_tag_Y                  !== dbg_prev_opsum_tag_Y)          ||
            (PEA_opsum_issue_fire         !== dbg_prev_PEA_opsum_issue_fire) ||
            (FT_CONTROLLER.asic_controller_0.opsum_meta_count             !== dbg_prev_opsum_meta_count)     ||
            (PEA_opsum_valid              !== dbg_prev_PEA_opsum_valid)      ||
            (PEA_opsum_ready              !== dbg_prev_PEA_opsum_ready)      ||
            (PE_array.GON_opsum_valid     !== dbg_prev_GON_opsum_valid)      ||
            (PE_array.GON_opsum_ready     !== dbg_prev_GON_opsum_ready)      ||
            (PE_array.PE_opsum_valid[23:16] !== dbg_prev_PE_row2_valid)      ||
            (PE_array.PE_opsum_valid[47:40] !== dbg_prev_PE_row5_valid)) begin

            $display("[OPSUM_DEBUG][t=%0t] valid_e=%b PE_en=%b | tag_X=%0d tag_Y=%0d | issue_fire=%b meta_count=%0d | PEA_v=%b PEA_r=%b | GON_v=%b GON_r=%b | row2=%b row5=%b",
                     $time,
                     FT_CONTROLLER.asic_controller_0.valid_e,
                     PE_en,
                     opsum_tag_X,
                     opsum_tag_Y,
                     PEA_opsum_issue_fire,
                     FT_CONTROLLER.asic_controller_0.opsum_meta_count,
                     PEA_opsum_valid,
                     PEA_opsum_ready,
                     PE_array.GON_opsum_valid,
                     PE_array.GON_opsum_ready,
                     PE_array.PE_opsum_valid[23:16],
                     PE_array.PE_opsum_valid[47:40]);
        end

        dbg_prev_valid_e              <= FT_CONTROLLER.asic_controller_0.valid_e;
        dbg_prev_PE_en                <= PE_en;
        dbg_prev_opsum_tag_X          <= opsum_tag_X;
        dbg_prev_opsum_tag_Y          <= opsum_tag_Y;
        dbg_prev_PEA_opsum_issue_fire <= PEA_opsum_issue_fire;
        dbg_prev_opsum_meta_count     <= FT_CONTROLLER.asic_controller_0.opsum_meta_count;
        dbg_prev_PEA_opsum_valid      <= PEA_opsum_valid;
        dbg_prev_PEA_opsum_ready      <= PEA_opsum_ready;

        dbg_prev_GON_opsum_valid      <= PE_array.GON_opsum_valid;
        dbg_prev_GON_opsum_ready      <= PE_array.GON_opsum_ready;
        dbg_prev_PE_row2_valid        <= PE_array.PE_opsum_valid[23:16];
        dbg_prev_PE_row5_valid        <= PE_array.PE_opsum_valid[47:40];
    end
end*/
