...
class VisuGraph(object):
    ...
    def init_node(self):
        ...
        for k, v in self.nodes.items():
            ...
            self.node_code += (
                "graph.node(name='{}', label='{}', color='{}', style='{}', fixedsize='{}', height='{}')\n".format(
                    k, v.label, v.color, v.style, v.fixedsize, v.height
                )
            )
    ...
    def init_edge(self):
        ...
        for edge in self.edges:
            ...
            self.edge_code += "graph.edge(tail_name='{}', head_name='{}', label='{}')\n".format(
                edge.tail_name, edge.head_name, edge.shape
            )
        ...
        for k, v in pnodes.items():
            ...
            for ops_ in ops_list:
                if " = " in ops_:
                    ...
                    self.nodes[match_op[0]] = IRNode(
                        name=match_op[0],
                        label=match_op[-1][:index],
                        inputs=args_list,
                        color=color,
                        style="filled",
                    )
                    ...
                    self.nodes[k] = IRNode(
                        name=k,
                        label=ops_[:index] if index != -1 else ops_,
                        inputs=args_list,
                        color=color,
                        style="filled",
                    )
    ...
    def split_fn_op(self):
        ...
        for output_str, body_str in self.parse_res:
            if "fn_" in body_str:
                ...
                # '%2 = subtract(%a, %b);'
                args = match_op[-1].split(", ")  # op的输入参数
                pnodes[output_str if output_str else "}"] = PNode(
                    name=output_str if output_str else "}",
                    type="op",
                    inputs=args,
                    body=match_op[0],
                )
