...
#define ALIGN_SIZE(x) (((x) + 63u) & ~63u)

#ifdef DLA_BITMAP_BACKEND
#define DLA_BITMAP_MAX_TILE_M 128u
namespace {
...
int qconv2d_relu_maxpool(
    ...
    uint32_t cand_m = 1u;
    while ((cand_m << 1) <= m && (cand_m << 1) <= M &&
           (cand_m << 1) <= DLA_BITMAP_MAX_TILE_M) {
        cand_m <<= 1;
    }
    ...
    /*! <<<========= Implement here =========>>> */
    uint8_t* ifmap_dma_ptr = input_in_DRAM;
    uint32_t ifmap_dma_size = ifmap_len;
    int8_t* filter_dma_ptr = filter_in_DRAM;
    uint32_t filter_dma_size = filter_len;
#ifdef DLA_BITMAP_BACKEND
    filter_dma_ptr = bitmap_filter_dma;
    filter_dma_size = (uint32_t)(bitmap_payload.compressed_filter.size() * sizeof(uint32_t));
#ifdef DLA_BITMAP_IFMAP
    ifmap_dma_ptr = bitmap_ifmap_dma;
    ifmap_dma_size = (uint32_t)(bitmap_payload.compressed_ifmap.size() * sizeof(uint32_t));
#endif
#endif

    uint32_t input_handle = mmu_map(ifmap_dma_ptr, ALIGN_SIZE(ifmap_dma_size));
    uint32_t filter_handle = mmu_map(filter_dma_ptr, ALIGN_SIZE(filter_dma_size));
    uint32_t bias_handle = mmu_map(bias, ALIGN_SIZE(M * sizeof(int32_t)));
    uint32_t output_handle = mmu_map(opsum_in_DRAM, ALIGN_SIZE(ofmap_len));

    set_ifmap_addr((uint8_t*)(uint64_t)input_handle);
    set_filter_addr((int8_t*)(uint64_t)filter_handle);
    set_bias_addr((int32_t*)(uint64_t)bias_handle);
    set_opsum_addr((uint8_t*)(uint64_t)output_handle);
    ...
    fprintf(stderr,
            "[DLA/E2E] start qconv2d_relu_maxpool m=%u e=%u p=%u q=%u r=%u t=%u PAD=%u U=%u R=%u S=%u C=%u M=%u W=%u H=%u scale=%u\n",
            m, e, p, q, r, t, PAD, U, R, S, C, M, W, H, scale);
    set_enable(scale, true, true, false);
    wait_for_interrupt();
    dla_stop();
    fprintf(stderr, "[DLA/E2E] done qconv2d_relu_maxpool\n");

    mmu_unmap(input_handle);
    mmu_unmap(filter_handle);
    mmu_unmap(bias_handle);
    mmu_unmap(output_handle);
#ifdef DLA_INFO
...
int qconv2d_relu(uint8_t* input_in_DRAM, int8_t* filter_in_DRAM,
    ...
    // set_enable(scale_factor, maxpool, relu, operation)
    fprintf(stderr,
            "[DLA/E2E] start qconv2d_relu m=%u e=%u p=%u q=%u r=%u t=%u PAD=%u U=%u R=%u S=%u C=%u M=%u W=%u H=%u scale=%u\n",
            m, e, p, q, r, t, PAD, U, R, S, C, M, W, H, scale);
    set_enable(scale, false, true, false);

    wait_for_interrupt();
    dla_stop();
    fprintf(stderr, "[DLA/E2E] done qconv2d_relu\n");
