...
*****************************************/
  ...
  logic [`DATA_BITS-1:0] ASIC_SPARSE_BITMAP_INDEX, ASIC_SPARSE_BITMAP_INDEX_next;
  logic [`DATA_BITS-1:0] ASIC_SPARSE_LEN_INDEX, ASIC_SPARSE_LEN_INDEX_next;
  logic [`DATA_BITS-1:0] ASIC_SPARSE_CTRL, ASIC_SPARSE_CTRL_next;

  logic sparse_bitmap_wr_en;
  logic [1:0] sparse_bitmap_wr_sel;
  logic [`SPARSE_BMAP_ADDR_BITS-1:0] sparse_bitmap_wr_addr;
  logic [`SPARSE_BLOCK_SIZE-1:0] sparse_bitmap_wr_data;
  logic [`SPARSE_COUNT_BITS-1:0] sparse_bitmap_wr_nz_count;
  logic [1:0] sparse_bitmap_rd_sel;
  logic [`SPARSE_BMAP_ADDR_BITS-1:0] sparse_bitmap_rd_addr;
  logic [`SPARSE_BLOCK_SIZE-1:0] sparse_bitmap_rd_data;
  logic [`SPARSE_COUNT_BITS-1:0] sparse_bitmap_rd_nz_count;
  logic sparse_len_wr_en;
  logic [1:0] sparse_len_wr_sel;
  logic [`SPARSE_BMAP_ADDR_BITS-1:0] sparse_len_wr_addr;
  logic [`GLB_ADDR_BITS-1:0] sparse_len_wr_data;
  logic [1:0] sparse_len_rd_sel;
  logic [`SPARSE_BMAP_ADDR_BITS-1:0] sparse_len_rd_addr;
  logic [`GLB_ADDR_BITS-1:0] sparse_len_rd_data;
  ...
  always_ff @(posedge ACLK) begin
    if (~ARESETn) begin
      ...
      ASIC_SPARSE_BITMAP_INDEX <= `DATA_BITS'd0;
      ASIC_SPARSE_LEN_INDEX <= `DATA_BITS'd0;
      ASIC_SPARSE_CTRL <= `DATA_BITS'd0;
    end
    else begin
      ...
      ASIC_SPARSE_BITMAP_INDEX <= ASIC_SPARSE_BITMAP_INDEX_next;
      ASIC_SPARSE_LEN_INDEX <= ASIC_SPARSE_LEN_INDEX_next;
      ASIC_SPARSE_CTRL <= ASIC_SPARSE_CTRL_next;
    end
  end
  ...
  always_comb begin
    ...
    ASIC_SPARSE_BITMAP_INDEX_next = ASIC_SPARSE_BITMAP_INDEX;
    ASIC_SPARSE_LEN_INDEX_next = ASIC_SPARSE_LEN_INDEX;
    ASIC_SPARSE_CTRL_next = ASIC_SPARSE_CTRL;
    sparse_bitmap_wr_en = 1'b0;
    sparse_bitmap_wr_sel = ASIC_SPARSE_BITMAP_INDEX[17:16];
    sparse_bitmap_wr_addr = ASIC_SPARSE_BITMAP_INDEX[`SPARSE_BMAP_ADDR_BITS-1:0];
    sparse_bitmap_wr_data = WDATA_S[`SPARSE_BLOCK_SIZE-1:0];
    sparse_bitmap_wr_nz_count = WDATA_S[11:8];
    sparse_bitmap_rd_sel = ASIC_SPARSE_BITMAP_INDEX[17:16];
    sparse_bitmap_rd_addr = ASIC_SPARSE_BITMAP_INDEX[`SPARSE_BMAP_ADDR_BITS-1:0];
    sparse_len_wr_en = 1'b0;
    sparse_len_wr_sel = ASIC_SPARSE_LEN_INDEX[17:16];
    sparse_len_wr_addr = ASIC_SPARSE_LEN_INDEX[`SPARSE_BMAP_ADDR_BITS-1:0];
    sparse_len_wr_data = WDATA_S[`GLB_ADDR_BITS-1:0];
    sparse_len_rd_sel = ASIC_SPARSE_LEN_INDEX[17:16];
    sparse_len_rd_addr = ASIC_SPARSE_LEN_INDEX[`SPARSE_BMAP_ADDR_BITS-1:0];
    ...
    case (cs_slave)
      ...
      READ_DATA: begin
        ...
        case (addr_S_reg)
          ...
          `ASIC_MAPPING_PARAM_OFFSET: begin
            RDATA_S = ASIC_MAPPING_PARAM;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_SHAPE_PARAM1_OFFSET: begin
            RDATA_S = ASIC_SHAPE_PARAM1;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_SHAPE_PARAM2_OFFSET: begin
            RDATA_S = ASIC_SHAPE_PARAM2;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_IFMAP_ADDR_OFFSET: begin
            RDATA_S = ASIC_IFMAP_ADDR;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_FILTER_ADDR_OFFSET: begin
            RDATA_S = ASIC_FILTER_ADDR;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_BIAS_ADDR_OFFSET: begin
            RDATA_S = ASIC_BIAS_ADDR;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_OPSUM_ADDR_OFFSET: begin
            RDATA_S = ASIC_OPSUM_ADDR;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_GLB_FILTER_ADDR_OFFSET: begin
            RDATA_S = ASIC_GLB_FILTER_ADDR;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_GLB_OFMAP_ADDR_OFFSET: begin
            RDATA_S = ASIC_GLB_OFMAP_ADDR;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_GLB_BIAS_ADDR_OFFSET: begin
            RDATA_S = ASIC_GLB_BIAS_ADDR;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_IFMAP_LEN_OFFSET: begin
            RDATA_S = ASIC_IFMAP_LEN;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_OFMAP_LEN_OFFSET: begin
            RDATA_S = ASIC_OFMAP_LEN;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_SPARSE_BITMAP_INDEX_OFFSET: begin
            RDATA_S = ASIC_SPARSE_BITMAP_INDEX;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_SPARSE_BITMAP_RDATA_OFFSET: begin
            RDATA_S = {{(`DATA_BITS-12){1'b0}}, sparse_bitmap_rd_nz_count, sparse_bitmap_rd_data};
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_SPARSE_LEN_INDEX_OFFSET: begin
            RDATA_S = ASIC_SPARSE_LEN_INDEX;
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_SPARSE_LEN_RDATA_OFFSET: begin
            RDATA_S = {{(`DATA_BITS-`GLB_ADDR_BITS){1'b0}}, sparse_len_rd_data};
            RRESP_S = `AXI_RESP_OKAY;
          end
          `ASIC_SPARSE_CTRL_OFFSET: begin
            RDATA_S = ASIC_SPARSE_CTRL;
            RRESP_S = `AXI_RESP_OKAY;
          end
          default: begin
      ...
      WRITE_DATA: begin // write data
        ...
        if (WVALID_S) begin
          ...
          end else if(addr_S_reg == `ASIC_SPARSE_BITMAP_INDEX_OFFSET) begin
            ASIC_SPARSE_BITMAP_INDEX_next = WDATA_S;
          end else if(addr_S_reg == `ASIC_SPARSE_BITMAP_WDATA_OFFSET) begin
            sparse_bitmap_wr_en = 1'b1;
          end else if(addr_S_reg == `ASIC_SPARSE_LEN_INDEX_OFFSET) begin
            ASIC_SPARSE_LEN_INDEX_next = WDATA_S;
          end else if(addr_S_reg == `ASIC_SPARSE_LEN_WDATA_OFFSET) begin
            sparse_len_wr_en = 1'b1;
          end else if(addr_S_reg == `ASIC_SPARSE_CTRL_OFFSET) begin
            ASIC_SPARSE_CTRL_next = WDATA_S;
          end else begin
...
asic #(
    ...
    .ifmap_sparse_en_i(ASIC_SPARSE_CTRL[0]),
    ...
    .sparse_bitmap_wr_en(sparse_bitmap_wr_en),
    .sparse_bitmap_wr_sel(sparse_bitmap_wr_sel),
    .sparse_bitmap_wr_addr(sparse_bitmap_wr_addr),
    .sparse_bitmap_wr_data(sparse_bitmap_wr_data),
    .sparse_bitmap_wr_nz_count(sparse_bitmap_wr_nz_count),
    .sparse_bitmap_rd_sel(sparse_bitmap_rd_sel),
    .sparse_bitmap_rd_addr(sparse_bitmap_rd_addr),
    .sparse_bitmap_rd_data(sparse_bitmap_rd_data),
    .sparse_bitmap_rd_nz_count(sparse_bitmap_rd_nz_count),
    .sparse_len_wr_en(sparse_len_wr_en),
    .sparse_len_wr_sel(sparse_len_wr_sel),
    .sparse_len_wr_addr(sparse_len_wr_addr),
    .sparse_len_wr_data(sparse_len_wr_data),
    .sparse_len_rd_sel(sparse_len_rd_sel),
    .sparse_len_rd_addr(sparse_len_rd_addr),
    .sparse_len_rd_data(sparse_len_rd_data),
