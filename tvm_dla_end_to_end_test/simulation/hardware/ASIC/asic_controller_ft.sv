...
module asic_controller_ft #(
    parameter OPSUM_META_FIFO_DEPTH = 8
) (
    input clk,
    input rst,
    input asic_en,
    ...
    output logic PEA_opsum_issue_valid,
    output logic PEA_opsum_issue_ready,
    input PEA_opsum_issue_fire,
    output logic [`XID_BITS-1:0] opsum_tag_X,
    output logic [`YID_BITS-1:0] opsum_tag_Y,
    output logic [`XID_BITS-1:0] opsum_next_tag_X,
    output logic [`YID_BITS-1:0] opsum_next_tag_Y,
    output logic opsum_next_tag_valid,
    ...
    output logic [1:0] sparse_read_sel,
...
/* declare signals */
    ...
    localparam OPSUM_META_PTR_BITS = (OPSUM_META_FIFO_DEPTH <= 2)? 1: $clog2(OPSUM_META_FIFO_DEPTH);

    logic [2:0] issue_count_PP_filt;
    logic [2:0] issue_count_e;
    logic issue_count_PP_filt_reset;
    logic issue_count_e_reset;
    logic issue_last;
    logic [2:0] next_issue_count_PP_filt;
    logic [2:0] next_issue_count_e;
    logic store_issue_done;
    logic opsum_commit_fire;
    logic store_all_done;
    logic opsum_meta_fifo_full;
    logic opsum_meta_fifo_empty;
    logic opsum_meta_fifo_push;
    logic opsum_meta_fifo_pop;
    logic [OPSUM_META_PTR_BITS-1:0] opsum_meta_wr_ptr;
    logic [OPSUM_META_PTR_BITS-1:0] opsum_meta_rd_ptr;
    logic [OPSUM_META_PTR_BITS:0] opsum_meta_count;
    logic [`GLB_ADDR_BITS-1:0] opsum_meta_addr [0:OPSUM_META_FIFO_DEPTH-1];
    logic opsum_meta_last [0:OPSUM_META_FIFO_DEPTH-1];
    logic [`GLB_ADDR_BITS-1:0] opsum_issue_addr;
    logic [`AXI_ADDR_BITS-1:0] opsum_issue_mul1;
    logic [`AXI_ADDR_BITS-1:0] opsum_issue_mul2;
    logic [`AXI_ADDR_BITS-1:0] opsum_issue_addr_wide;
...
/* ================================================================================================= */
    ...
    assign DMA_byte_bias = (ifmap_sparse_en && cs == LOAD_IFMAP) ? 2'd0 : count_c;
...
/* ================================================================================================= */
    ...
    assign GLB_EN = ~(cs == SEND_AIFMAP || cs == SEND_IFMAP || cs == SEND_AFILT || cs == SEND_FILT
                       || cs == SEND_AIPSUM || cs == SEND_IPSUM || (cs == STORE_OPSUM && opsum_commit_fire)
                       || cs == PASS_PPU || cs == STORE_OFMAP);
    assign GLB_WEB = ~((cs == STORE_OPSUM && opsum_commit_fire) || cs == STORE_OFMAP);
    always_comb  begin // GLB_A
        ...
        case(cs)
            ...
            STORE_OPSUM: GLB_A = (opsum_meta_fifo_empty)? 'd0: opsum_meta_addr[opsum_meta_rd_ptr];
    ...
    always_comb begin
        case (cs)
            SEND_AIFMAP, SEND_IFMAP: sparse_read_sel = ifmap_sparse_en ? `SPARSE_READ_IFMAP : `SPARSE_READ_NONE;
            SEND_AFILT, SEND_FILT: sparse_read_sel = `SPARSE_READ_FILTER;
            default: sparse_read_sel = `SPARSE_READ_NONE;
        endcase
    end
...
/* ================================================================================================= */
    ...
    assign count_c_reset = ifmap_sparse_en ? 1'b1 : (count_c == PP_ch - 2'd1);
    ...
    assign count_PP_filt_reset = ({3'd0, count_PP_filt} >= PP_filt - 6'd1);
    ...
    always_ff @( posedge clk ) begin // count_PP_filt
        if (rst) count_PP_filt <= 'd0;
            ...
            else if (cs == STORE_OPSUM && opsum_commit_fire) begin
                if (count_PP_filt_reset) count_PP_filt <= 'd0;
                else count_PP_filt <= count_PP_filt + 'd1;
            end
    ...
    assign count_F_reset = (cs == STORE_OFMAP)?((maxpool)? (count_W >= F[7:1] - 8'd1): (count_W >= F - 8'd1)): (count_W >= W - 8'd1);
    always_ff @( posedge clk ) begin // count_W
        if (rst) count_W <= 'd0;
            ...
            if (cs == STORE_OPSUM && opsum_commit_fire && count_PP_filt_reset && count_e_reset) begin
    ...
    assign count_e_reset = (cs == STORE_OFMAP && maxpool)? ({1'b0, count_e} >= {1'b0, valid_e[3:1]} - 4'd1): ({1'b0, count_e} >= valid_e - 4'd1);
    always_ff @( posedge clk ) begin // count_e
        if (rst) count_e <= 3'd0;
        else begin
            ...
            else if (cs == STORE_OPSUM && opsum_commit_fire && count_PP_filt_reset) begin
                if (count_e_reset) count_e <= 3'd0;
                else count_e <= count_e + 3'd1;
            end
    ...
    assign issue_count_PP_filt_reset = (issue_count_PP_filt == PP_filt - 4'd1);
    assign issue_count_e_reset = (issue_count_e == valid_e - 'd1);
    assign issue_last = issue_count_PP_filt_reset && issue_count_e_reset;
    assign next_issue_count_PP_filt = (issue_count_PP_filt_reset)? 'd0: issue_count_PP_filt + 'd1;
    assign next_issue_count_e = (issue_count_PP_filt_reset)?
                                ((issue_count_e_reset)? 'd0: issue_count_e + 'd1):
                                issue_count_e;

    always_ff @( posedge clk ) begin
        if (rst) begin
            issue_count_PP_filt <= 'd0;
            issue_count_e <= 'd0;
            store_issue_done <= 1'b0;
        end
        else begin
            if (cs != STORE_OPSUM) begin
                issue_count_PP_filt <= 'd0;
                issue_count_e <= 'd0;
                store_issue_done <= 1'b0;
            end
            else if (PEA_opsum_issue_fire) begin
                issue_count_PP_filt <= next_issue_count_PP_filt;
                issue_count_e <= next_issue_count_e;
                if (issue_last) store_issue_done <= 1'b1;
            end
        end
    end

    assign opsum_issue_mul1 = issue_count_e * F;
    assign opsum_issue_mul2 = eF * (count_m + issue_count_PP_filt);
    assign opsum_issue_addr_wide = GLB_opsum_addr + {(count_F + opsum_issue_mul1 + opsum_issue_mul2), 2'd0};
    assign opsum_issue_addr = opsum_issue_addr_wide[`GLB_ADDR_BITS-1:0];

    assign opsum_meta_fifo_full = (opsum_meta_count == OPSUM_META_FIFO_DEPTH[OPSUM_META_PTR_BITS:0]);
    assign opsum_meta_fifo_empty = (opsum_meta_count == 'd0);
    assign opsum_meta_fifo_push = PEA_opsum_issue_fire;
    assign opsum_meta_fifo_pop = opsum_commit_fire;
    assign opsum_commit_fire = PEA_opsum_valid && PEA_opsum_ready;
    assign store_all_done = store_issue_done && opsum_commit_fire && opsum_meta_last[opsum_meta_rd_ptr];

    always_ff @( posedge clk ) begin
        if (rst || cs != STORE_OPSUM) begin
            opsum_meta_wr_ptr <= 'd0;
            opsum_meta_rd_ptr <= 'd0;
            opsum_meta_count <= 'd0;
            for (int fifo_idx = 0; fifo_idx < OPSUM_META_FIFO_DEPTH; fifo_idx = fifo_idx + 1) begin
                opsum_meta_addr[fifo_idx] <= 'd0;
                opsum_meta_last[fifo_idx] <= 1'b0;
            end
        end
        else begin
            if (opsum_meta_fifo_push) begin
                opsum_meta_addr[opsum_meta_wr_ptr] <= opsum_issue_addr;
                opsum_meta_last[opsum_meta_wr_ptr] <= issue_last;
                if (opsum_meta_wr_ptr == OPSUM_META_FIFO_DEPTH-1) opsum_meta_wr_ptr <= 'd0;
                else opsum_meta_wr_ptr <= opsum_meta_wr_ptr + 'd1;
            end
            if (opsum_meta_fifo_pop) begin
                if (opsum_meta_rd_ptr == OPSUM_META_FIFO_DEPTH-1) opsum_meta_rd_ptr <= 'd0;
                else opsum_meta_rd_ptr <= opsum_meta_rd_ptr + 'd1;
            end

            case ({opsum_meta_fifo_push, opsum_meta_fifo_pop})
                2'b10: opsum_meta_count <= opsum_meta_count + 'd1;
                2'b01: opsum_meta_count <= opsum_meta_count - 'd1;
                default: opsum_meta_count <= opsum_meta_count;
            endcase
        end
    end
...
/* ================================================================================================= */
    ...
    assign opsum_tag_X = (cs == STORE_OPSUM && !store_issue_done)? issue_count_e: `DEFAULT_XID;
    assign opsum_tag_Y = (cs == STORE_OPSUM && !store_issue_done)? (issue_count_PP_filt > 3'd3)? 3'd1: 3'd0: `DEFAULT_YID;
    assign opsum_next_tag_X = next_issue_count_e;
    assign opsum_next_tag_Y = (next_issue_count_PP_filt > 3'd3)? 3'd1: 3'd0;
    assign opsum_next_tag_valid = (cs == STORE_OPSUM && !store_issue_done && !issue_last);
...
/* ================================================================================================= */
    always_comb  begin
        if (cs == SEND_PE_CONFIG) begin
            case (valid_e)
                4'd1: PE_en = {6{8'b00000001}};
                4'd2: PE_en = {6{8'b00000011}};
                4'd3: PE_en = {6{8'b00000111}};
                4'd4: PE_en = {6{8'b00001111}};
                4'd5: PE_en = {6{8'b00011111}};
                4'd6: PE_en = {6{8'b00111111}};
                4'd7: PE_en = {6{8'b01111111}};
...
/* ================================================================================================= */
    ...
    assign PEA_opsum_ready = (cs == STORE_OPSUM && !opsum_meta_fifo_empty)? 1'b1: 1'b0;
    assign PEA_opsum_issue_valid = (cs == STORE_OPSUM && !store_issue_done)? 1'b1: 1'b0;
    assign PEA_opsum_issue_ready = (cs == STORE_OPSUM && !store_issue_done && !opsum_meta_fifo_full)? 1'b1: 1'b0;
