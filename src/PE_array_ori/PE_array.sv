`include "../src/PE_array_ori/PE.sv"
`include "../src/PE_array_ori/GIN/GIN.sv"
`include "../src/PE_array_ori/GON/GON.sv"
`include "../define.svh"
...
always_ff @( posedge clk or posedge rst ) begin //LN_config
    if (rst) LN_config <= 'd0;
    else begin
...
assign PE_ipsum[NUMS_PE_COL*DATA_SIZE-1:0] = {NUMS_PE_COL{GIN_ipsum}};
    ...
    for (i = 1; i < NUMS_PE_ROW; i++) begin
        always_comb begin
            if (LN_config[i-1]) begin
                ...
                PE_ipsum[((i+1)*NUMS_PE_COL*DATA_SIZE-1):(i*NUMS_PE_COL*DATA_SIZE)] = {NUMS_PE_COL{GIN_ipsum}};
